...
// import libraries for OpenZeppelin's SafeMath and Oraclize
...
contract BiteChain is usingOraclize  {
    ...
    /// @notice Callback for use by Oracalize only
    /// @dev Call out is in update() then returned info comes here
    /// @param myid Internal use for Oracalize request ID
    /// @param result Actual info returned from WolframAlpha - Oracalize
    function __callback(bytes32 myid, string memory result) public {
        require(msg.sender == oraclize_cbAddress(),"Message must come from Oracle");
        ethPrice = result;
        oracleID = myid;
    }

    /// @notice Updates the USD-ETH Price using oracle WolframAlpha
    /// @dev Calls cost ETH, so this is payable just in case contract is out of money
    function update() public payable {
        oraclize_query("WolframAlpha", "ethereum cost in usd");
    }
